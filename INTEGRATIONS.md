...
**cognis-arsenal** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cognis-arsenal` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cognis-arsenal ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cognis-arsenal ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
cognis-arsenal ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cognis-arsenal` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cognis-arsenal") for rec in records]   # records = your JSON output
